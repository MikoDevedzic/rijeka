// SPRINT_4G_3_pricer_page.js
// Complete replacement of PricerPage.jsx with:
//   - Curve panel: snapshot pillars table, parallel bump, per-tenor bump
//   - Cashflow table: Pay Date | Leg | Dir | Fwd Rate | Zero Rate | DF | DCF | Amount | PV
// Run from Rijeka root: node SPRINT_4G_3_pricer_page.js

const fs = require('fs');
const FILE = 'C:\\Users\\mikod\\OneDrive\\Desktop\\Rijeka\\frontend\\src\\components\\pricer\\PricerPage.jsx';

const content = `// PricerPage.jsx — Sprint 4G (curve bump + transparency)
...
function CurveBumpPanel({ curveIds, snapshotsByid, bumps, onBumpParallel, onBumpTenor, onToggleOverride, overrides, onRateChange }) {
                      ...
                      {snap.quotes.filter(q => q.enabled !== false).map((q, i) => {
                        ...
                        return (
                          <tr key={i} className={hasBump ? 'bumped' : ''}>
                            ...
                            <td className="pp-dim">{q.quote_type}</td>
        ...
        const quotes = snap.quotes
          ...
          .map(q => {
            ...
            return {
              ...
              rate:       (q.rate + totalBump) / 100, // % → decimal
            };
          });
        ...
        <div className="pp-right">
          ...
          {result && (
            <>
              ...
              {/* Curve pillars used */}
              {Object.keys(curvePillars).length > 0 && (
                <div className="pp-pillars-used">
                  <div className="pp-section-hdr">CURVES USED IN PRICING</div>
                  {Object.entries(curvePillars).map(([cid, pils]) => (
                    <div key={cid} className="pp-pillar-block">
                      <div className="pp-pillar-block-hdr">{cid} · {pils.length} pillars</div>
                      <table className="pp-pillar-display">
                        <thead>
                          <tr><th>MATURITY</th><th className="r">T (yrs)</th><th className="r">ZERO RATE</th><th className="r">DF</th></tr>
                        </thead>
                        <tbody>
                          {pils.map((p, i) => (
                            <tr key={i}>
                              <td className="pp-mono pp-dim">{p.date}</td>
                              <td className="r pp-mono pp-dim">{p.t.toFixed(2)}</td>
                              <td className="r pp-mono" style={{color:'var(--blue)'}}>{p.zero_rate.toFixed(4)}%</td>
                              <td className="r pp-mono pp-dim">{p.df.toFixed(6)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
...
console.log('Done. PricerPage.jsx replaced with bump + transparency version.');
